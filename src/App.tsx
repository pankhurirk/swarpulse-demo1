...
const PRACTICE_STORAGE_KEY = "swarpulse_practice_notes";
...
function parseNoteToken(token: string, defaultDuration: number) {
  ...
  if (swarPart.includes(":")) {
    const parts = swarPart.split(":");
    swarPart = parts[0].trim();
    duration = Number(parts[1].trim());
  }
...
export default function App() {
  ...
  const [savedPractice, setSavedPractice] = useState<PracticeItem[]>(() => {
    try {
  ...
  const tanpuraAudioRef = useRef<HTMLAudioElement | null>(null);
  const percussionAudioRef = useRef<HTMLAudioElement | null>(null);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(PRACTICE_STORAGE_KEY, JSON.stringify(savedPractice));
    } catch {}
  }, [savedPractice]);
  ...
  function stopAllAudio() {
    ...
    if (tanpuraAudioRef.current) {
      ...
      tanpuraAudioRef.current.currentTime = 0;
      tanpuraAudioRef.current = null;
    }

    if (percussionAudioRef.current) {
      percussionAudioRef.current.pause();
      percussionAudioRef.current.currentTime = 0;
      percussionAudioRef.current = null;
    }
  ...
  function stopTanpuraOnly() {
    ...
    setTanpuraOn(false);
    setStatus("Tanpura stopped");
    setCurrentNote("-");
  }

  function stopBeatOnly() {
    if (percussionAudioRef.current) {
      percussionAudioRef.current.pause();
      percussionAudioRef.current.currentTime = 0;
      percussionAudioRef.current = null;
  ...
  async function playLoop(path: string, label: string, isTanpura: boolean) {
    try {
      const audio = new Audio(path);
      audio.loop = true;
      const volumeValue = isTanpura ? Number(tanpuraVolume) : Number(percussionVolume);
      audio.volume = Math.max(0, Math.min(1, isNaN(volumeValue) ? 0.8 : volumeValue));
      await audio.play();
      ...
      if (isTanpura) {
        ...
        setTanpuraOn(true);
        setCurrentNote("Tanpura");
      } else {
        if (percussionAudioRef.current) {
          percussionAudioRef.current.pause();
          percussionAudioRef.current.currentTime = 0;
        }
        percussionAudioRef.current = audio;
        setCurrentNote("Beat");
      }
          ...
          <div style={narrow ? styles.grid1 : styles.grid2}>
            <div style={styles.card}>
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={styles.label}>Saved Practice Notes</label>
                <select
                  style={styles.select}
                  value=""
                  onChange={(e) => {
                    const notes = e.target.value;
                    if (notes) loadPractice(notes, "playback");
                  }}
                >
                  <option value="">Select a saved practice note</option>
                  {savedPractice.map((item, idx) => (
                    <option key={item.title + "-" + idx} value={item.notes}>{item.title}</option>
                  ))}
                </select>
              </div>
        ...
        {activeTab === "tanpura" && (
          <div style={narrow ? styles.grid1 : styles.grid2}>
            <div style={styles.card}>
              ...
              <div style={{ height: 12 }} />
              <label style={styles.label}>Tanpura Volume (0 to 1)</label>
              <input style={styles.input} value={tanpuraVolume} onChange={(e) => setTanpuraVolume(e.target.value)} />
              <div style={{ height: 12 }} />
              <label style={styles.label}>Percussion Volume (0 to 1)</label>
              <input style={styles.input} value={percussionVolume} onChange={(e) => setPercussionVolume(e.target.value)} />
              <div style={{ marginTop: 12 }}>
                <button style={styles.primaryButton} disabled={expired} onClick={playTanpura}>Start Tanpura</button>
                <button style={styles.button} onClick={stopTanpuraOnly}>Stop Tanpura</button>
              </div>
            ...
            <div style={styles.card}>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <button style={styles.button} onClick={stopBeatOnly}>Stop Beat</button>
              </div>
            </div>
          </div>
        )}
