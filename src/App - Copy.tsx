...
const demoPractice: PracticeItem[] = [
...
export default function App() {
  ...
  const [savedPractice, setSavedPractice] = useState<PracticeItem[]>(demoPractice);
  ...
  async function loadManifests() {
    try {
      ...
      setPercussionList(Array.isArray(pdata) ? pdata : []);

      const t = await fetch("/audio/tanpura/manifest.json");
      const tdata = await t.json();
      setTanpuraList(Array.isArray(tdata) ? tdata : []);

      if (Array.isArray(pdata) && pdata.length > 0) {
        setSelectedBeat(pdata[0].file);
      }
  ...
  async function playLoop(path: string, label: string) {
    stopAllAudio();
    try {
      const ctx = await ensureAudio();
  ...
  async function playPercussion() {
    if (!selectedBeat || expired) return;
    await playLoop(`/audio/percussion/${selectedBeat}`, selectedBeat);
    setCurrentNote("Beat");
  }

  async function playTanpura() {
    if (expired) return;
    if (tanpuraList.length === 0) {
      setStatus("No tanpura file");
      return;
    }
    const file = tanpuraList[0].file;
    await playLoop(`/audio/tanpura/${file}`, file);
    setTanpuraOn(true);
  }
  ...
  async function playSequence(sequence: string, mode: "synth" | "harmonium") {
    ...
    for (let loop = 1; loop <= repeats; loop += 1) {
      for (const token of tokens) {
        ...
        const octaveMultiplier = parsed.octaveMultiplier;
        const tokenDuration = parsed.duration;

        if (!BASE_SWARAS[note]) continue;

        const freq = base * BASE_SWARAS[note] * octaveMultiplier;

        setTimeout(() => setCurrentNote(token), Math.max(0, (cursor - ctx.currentTime) * 1000));
        setTimeout(() => {
          setStatus(`Loop ${loop}: ${mode === "synth" ? "Synth" : "Harmonium"} ${freq.toFixed(2)} Hz for ${tokenDuration.toFixed(2)}s`);
        }, Math.max(0, (cursor - ctx.currentTime) * 1000));

        await playVoice({
          freq,
          startAt: cursor,
          durationSec: tokenDuration,
        ...
        cursor += tokenDuration + GAP_SECONDS;
      }
    }
  ...
  const styles = {
    ...
    tab: (active: boolean): React.CSSProperties => ({
      background: active ? ACCENT : PANEL2,
      ...
      <div style={styles.wrap}>
        <div style={styles.header}>
          ...
          <div style={{ ...styles.small, marginTop: 8 }}>
            7-day demo: {expired ? "Expired" : formatRemaining(remainingMs)}
          </div>
          <div style={{ marginTop: 12 }}>
            <button style={styles.button} onClick={resetTrial}>Reset Demo Timer</button>
            <button style={styles.button} onClick={loadManifests}>Reload Audio Manifests</button>
        ...
        {activeTab === "playback" && (
          <div style={isNarrow ? styles.grid2Mobile : styles.grid2}>
            <div>
              <div style={styles.card}>
                <div style={styles.title}>Playback Settings</div>
                ...
                <div style={styles.panel}>
                  <div style={styles.small}>Playback tab is for Synth playback only.</div>
                </div>
                ...
                {savedPractice.map((item, idx) => (
                  <div key={`${item.title}-${idx}`} style={styles.panel}>
                    ...
                    <div style={{ marginTop: 10 }}>
                      <button style={styles.primaryButton} disabled={expired} onClick={() => loadPractice(item.notes, "playback")}>
                        Load to Playback
                      </button>
                    </div>
                  </div>
        ...
        {activeTab === "harmonium" && (
          <div style={styles.card}>
            ...
            <div style={styles.panel}>
              <div style={styles.small}>Harmonium demo mode is tuned around A# and uses a browser-generated reed-like tone.</div>
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={styles.label}>Enter Notes</label>
              <textarea
                style={styles.textarea}
                value={harmoniumNotes}
                onChange={(e) => setHarmoniumNotes(e.target.value)}
              />
            </div>
            <div>
              <button style={styles.primaryButton} disabled={expired} onClick={() => playSequence(harmoniumNotes, "harmonium")}>
                Play Harmonium
              </button>
              <button style={styles.button} onClick={stopAudioOnly}>Stop</button>
            ...
            <div style={styles.card}>
              ...
              <div style={styles.panel}>
                <div style={styles.small}>
                  Tanpura file is read from: <code>public/audio/tanpura/manifest.json</code>
                </div>
              </div>
              ...
              <div style={{ ...styles.small, marginTop: 12 }}>
                Current state: {tanpuraOn ? "Running" : "Stopped"}
              </div>
            </div>
            ...
            <div style={styles.card}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <select style={styles.select} value={selectedBeat} onChange={(e) => setSelectedBeat(e.target.value)}>
                  {percussionList.map((item) => (
                    <option key={item.file} value={item.file}>{item.name}</option>
                  ))}
                </select>
              </div>
              <div>
                <button style={styles.primaryButton} disabled={expired} onClick={playPercussion}>Play Beat</button>
                <button style={styles.button} onClick={stopAudioOnly}>Stop Beat</button>
              </div>
              <div style={{ ...styles.small, marginTop: 12 }}>
                Percussion file is read from: <code>public/audio/percussion/manifest.json</code>
              </div>
            </div>
          </div>
        )}
              ...
              {savedPractice.map((item, idx) => (
                <div key={`${item.title}-${idx}`} style={styles.panel}>
                  ...
                  <div style={{ marginTop: 10 }}>
                    ...
                    <button style={styles.button} disabled={expired} onClick={() => loadPractice(item.notes, "harmonium")}>
                      Load to Harmonium
                    </button>
                  </div>
                </div>
